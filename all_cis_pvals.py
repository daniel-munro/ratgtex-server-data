...
infile = args.indir / args.tissue / f'{args.tissue}.cis_qtl_all_pvals.txt.gz'
outfile = args.outdir / 'cis_pvals' / f'{args.tissue}.zip'
        ...
        for line in f:
            ...
            if gene != last_gene:
                ...
                if last_gene is not None:
                    out.writestr(f'{args.tissue}/{last_gene}.txt', pvals)
                pvals = 'variant_id\tpval_nominal\n'
                last_gene = gene
            pvals += f'{var}\t{pval}' # pval still has newline
        out.writestr(f'{args.tissue}/{gene}.txt', pvals)
