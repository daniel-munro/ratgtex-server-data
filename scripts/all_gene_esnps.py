"""Assemble all significant expression cis associations in zip archive of per-gene files

Inputs:
    {indir}/v4/{tissue}/{tissue}.expression.cis_qtl_signif.txt.gz
...
parser.add_argument("outdir", type=Path, help="Output directory path for this data version")
...
for tissue in args.tissues:
    print(tissue)
    d = pd.read_csv(args.indir / args.version / tissue / f"{tissue}.expression.cis_qtl_signif.txt.gz", sep="\t")
...
outfile = args.outdir / "eqtl" / f"all_gene_esnps.{args.version}.zip"
with zipfile.ZipFile(outfile, 'w', zipfile.ZIP_DEFLATED) as out:
    for gene, d in sig.groupby("phenotype_id"):
        ...
        out.writestr(f"all_gene_esnps.{args.version}/{gene}.txt", d_str)
